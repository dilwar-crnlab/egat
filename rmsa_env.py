...
class RMSAEnv(OpticalNetworkEnv):
    ...
    def compute_node_features(self, service):
        """
        Compute node features for nodes in candidate paths and their neighbors.
        Currently considers node betweenness centrality.
        """
        node_features = {}
        relevant_nodes = set()
        # Get paths for current service
        paths = self.k_shortest_paths[service.source, service.destination]
        # First collect all nodes in paths
        for path in paths:
        ...
        # Compute betweenness centrality for relevant nodes
        betweenness = nx.betweenness_centrality(self.topology)
        # Normalize betweenness values
        max_betweenness = max(betweenness.values())
        # Store normalized betweenness for each relevant node
        for node in relevant_nodes:
            node_features[node] = {
    ...
    def compute_path_edge_features(self, service):
        """
        Computes band-specific edge features for paths and neighboring edges.
        Returns dictionary with separate C and L band features.
        """
        path_features = {}
        relevant_edges = set()
        edge_features = {}
        ...
        # First collect all edges in paths
        for path_idx, path in enumerate(paths):
            for i in range(len(path.node_list) - 1):
                node1, node2 = path.node_list[i], path.node_list[i + 1]
                relevant_edges.add((node1, node2))
        ...
        for node1, node2 in relevant_edges:
            ...
            for node in [node1, node2]:
                for neighbor in self.topology.neighbors(node):
                    potential_edge = tuple(sorted([node, neighbor]))
                    if potential_edge not in relevant_edges and neighbor != node1 and neighbor != node2:
                        neighbor_edges.add(potential_edge)
                        
        all_edges = relevant_edges.union(neighbor_edges)
    
        def compute_band_features(edge, band_type):
            """
            ...
            if band_type == 'C':
                ...
                slots = self.topology.graph['available_slots'][link_idx + self.topology.number_of_edges(), 100:256]
                total_slots = 156
            ...
            features['utilization'] = 1.0 - (np.sum(slots) / total_slots)
            
            # 2. Available Blocks Information
            if len(available_indices) > 0:
                # First-fit block
                first_idx = available_indices[0]
                features['first_block'] = {
                    'position': (initial_indices[first_idx] + band_start) / total_slots,
                    'size': lengths[first_idx] / total_slots
                }
                
                # Last-fit block
                last_idx = available_indices[-1]
                features['last_block'] = {
                    'position': (initial_indices[last_idx] + band_start) / total_slots,
                    'size': lengths[last_idx] / total_slots
                }
                
                # All available blocks
                features['available_blocks'] = {
                    'positions': initial_indices[available_indices] + band_start,
                    'sizes': lengths[available_indices]
                }
            else:
                features['first_block'] = {'position': -1, 'size': 0}
                features['last_block'] = {'position': -1, 'size': 0}
                features['available_blocks'] = {'positions': [], 'sizes': []}
            
            # 3. Fragmentation Metrics
            features['fragmentation'] = self._compute_shannon_entropy(slots)
            
            # 4. Traffic Load
            active_connections = self.topology[node1][node2].get(active_conns, [])
            features['load'] = {
                'num_connections': len(active_connections) / self.episode_length,
                'avg_osnr_margin': self.compute_avg_osnr_margin(node1, node2, active_conns)
            }
            
            # 5. Convert to flat feature array for model input
            flat_features = np.array([
                features['utilization'],                    # [0] Utilization ratio
                features['first_block']['position'],        # [1] First-fit position
                features['first_block']['size'],            # [2] First-fit size
                features['last_block']['position'],         # [3] Last-fit position
                features['last_block']['size'],             # [4] Last-fit size
                features['fragmentation'],                  # [5] Fragmentation
                features['load']['num_connections'],        # [6] Number of active connections
                features['load']['avg_osnr_margin']         # [7] Average OSNR margin
            ])
            
            return {'features': flat_features, 'metadata': features}

        # Compute features for all edges
        for edge in all_edges:
            edge_features[edge] = {
                'C_band': compute_band_features(edge, 'C'),
                'L_band': compute_band_features(edge, 'L')
            }
        
        # Organize features by path
        for path_idx, path in enumerate(paths):
            path_edge_features = []
            for i in range(len(path.node_list) - 1):
                node1, node2 = path.node_list[i], path.node_list[i + 1]
                edge = (node1, node2)
                path_edge_features.append({
                    'C_band': edge_features[edge]['C_band']['features'],
                    'L_band': edge_features[edge]['L_band']['features']
                })
            path_features[path_idx] = np.array(path_edge_features)
        
        return path_features, edge_features
