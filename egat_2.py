...
class CustomActorCriticPolicy(ActorCriticPolicy):
    ...
    def _build_matrices(self, edge_features, node_indices, edge_indices):
        ...
        # Process each batch separately since each represents different service request
        for batch in range(batch_size):
            # First get all nodes in each path for this batch
            path_nodes = {}
            for i in range(self.k_paths):
                path_i = edge_features[batch,i,:,:]  # Get path features for this batch
                # Find valid nodes (non-zero features indicate presence of node)
                valid_nodes = torch.any(path_i != 0, dim=-1)
                nodes = set(torch.nonzero(valid_nodes).squeeze(-1).cpu().numpy())
                
                # Add 1-hop neighbors for this path's nodes
                neighbor_nodes = set()
                for node in nodes:
                    for neighbor in self.topology.neighbors(node):
                        neighbor_nodes.add(neighbor)
                
                # Store both path nodes and their neighbors
                path_nodes[i] = nodes.union(neighbor_nodes)
                # Print nodes and neighbors for first path
                if i == 0:
                    print("\nFor first candidate path:")
                    print("Path nodes:", nodes)
                    print("Neighbor nodes:", neighbor_nodes)
                    print("All relevant nodes (path + neighbors):", path_nodes[i])
        
            # Now check path connections including 1-hop neighbors for this batch
            for i in range(self.k_paths):
                for j in range(self.k_paths):
                    # Check if paths share any nodes (including neighbors)
                    if path_nodes[i].intersection(path_nodes[j]):
                        AH[batch,i,j] = 1
                        AH[batch,j,i] = 1  # Symmetric connection


        
        # Add self-loops
        AH.diagonal(dim1=1, dim2=2).fill_(1)
        print("\nAdjacency information for first path:")
        #print(f"First path (index 0) is connected to paths: {first_path_connections}")
        print("\nFull AH matrix for first batch:")
        print(AH[0].cpu().numpy())
...
def main():
    ...
    topology_path = f'{topology_name}_{k_paths}-paths_6-modulations.h5'
    
    # Print path for debugging
    print(f"Looking for topology file at: {topology_path}")
    
    try:
        with open(topology_path, 'rb') as f:
            topology = pickle.load(f)
    except FileNotFoundError:
        print(f"Topology file not found at {topology_path}")
        raise
